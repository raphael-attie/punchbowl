...
def test_sun_location_gcrs():
    ...
    # with frames.Helioprojective.assume_spherical_screen(skycoord_origin.observer):
    skycoord_origin_helio = skycoord_sun.transform_to(frames.Helioprojective(observer='earth', obstime=time_current))
...
def test_helio_to_celestial_wcs_many_points():
    for date_obs, crval1, crval2, crot in product(
        ...
        hp_coords = SkyCoord(hp_coords.Tx, hp_coords.Ty, 9e99*u.lyr, frame=hp_coords.frame)
        ...
        hp_to_eq_coords = hpc_to_gcrs(hp_coords)
        ...
        np.testing.assert_allclose(eq_ra, hp_to_eq_ra, atol=0.002)
        np.testing.assert_allclose(eq_coords.dec.to_value(u.deg), hp_to_eq_coords.dec.to_value(u.deg), atol=0.002)
...
def test_helio_to_celestial_wcs_many_points_3d():
    ...
    for date_obs, crval1, crval2, crot in product(
        ...
        hp_coords = SkyCoord(hp_coords[0].Tx, hp_coords[0].Ty, 9e99*u.lyr, frame=hp_coords[0].frame), hp_coords[1]
        # These come out claiming to be ICRS, but they're really GCRS (since it doesn't seem possible to tell a WCS
        # that it's GCRS)
        eq_coords = wcs_celestial.pixel_to_world(xs, ys, zs)
        hp_to_eq_coords = hpc_to_gcrs(hp_coords[0])
        ...
        np.testing.assert_allclose(eq_ra, hp_to_eq_ra, atol=0.002)
        np.testing.assert_allclose(eq_coords[0].dec.to_value(u.deg), hp_to_eq_coords.dec.to_value(u.deg), atol=0.002)
