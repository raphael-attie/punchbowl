...
def correct_vignetting_task(data_object: NDCube, vignetting_file: pathlib) -> NDCube:
    ...
    if vignetting_file is None:
        ...
        warnings.warn(f"Calibration file {vignetting_file} is unavailable, vignetting correction not applied", NoCalibrationDataWarning)
        ...
        if abs((vignetting_function_date - observation_date).to('day').value) > 14:
            warnings.warn(f"Calibration file {vignetting_file} contains data created greater than 2 weeks from the obsveration", LargeTimeDeltaWarning)
