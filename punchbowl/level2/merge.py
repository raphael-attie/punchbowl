...
def merge_many_task(data: list[NDCube], trefoil_wcs: WCS) -> NDCube:
    ...
    for polarization in [-60, 0, 60]:
        reprojected_data = np.stack([d.data for d in data if
            d.meta["POLAR"].value == polarization], axis=-1)
        reprojected_weights = np.stack([1/np.square(d.uncertainty.array)
            for d in data if d.meta["POLAR"].value == polarization], axis=-1)
        reprojected_weights[reprojected_weights <= 0] = 1E-8

        trefoil_data_layers.append(np.nansum(reprojected_data *
            reprojected_weights, axis=2) / np.nansum(reprojected_weights, axis=2))
