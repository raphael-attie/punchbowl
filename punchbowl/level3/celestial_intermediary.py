...
def to_celestial_frame_cutout(file: str | Path, cdelt: float = 0.02) -> tuple[np.ndarray, WCS]:
    """
    ...
    ----------
    file : str
        The input file to be reprojected
    cdelt : float
        The CDELT value to use for the output projection

    Returns
    -------
    reprojected_array : np.ndarray
        The cutout of the all-sky map containing the reprojected input data
    wcs : astropy.wcs.WCS
        The WCS describing the reprojected array

    """
    with fits.open(file) as hdul:
        data = hdul[1].data
        wcs = WCS(hdul[1].header, hdul, key="A").celestial
...
def shift_image_onto(source: np.ndarray,
                     target: np.ndarray,
                     source_wcs: WCS,
                     target_wcs: WCS,
    ...
    ----------
    source : np.ndarray
        The image array for the image to the aligned
    target : np.ndarray
        The image array onto which ``input`` is to be aligned
    source_wcs : astropy.wcs.WCS
        The WCS for the image being aligned
    target_wcs : astropy.wcs.WCS
        The WCS into which the first image is being aligned
    fill_value : float
    ...
    """
    if any(source_wcs.wcs.cdelt != target_wcs.wcs.cdelt):
    ...
    corner = utils.pixel_to_pixel(target_wcs, source_wcs, target.shape[-1] - 1, target.shape[-2] - 1)
    ...
    dcorner = corner[0] - (source.shape[-1] - 1), corner[1] - (source.shape[-2] - 1)
    if dcorner[0] < 0:
        source = source[..., :dcorner[0]]
    else:
        padding[1][1] = dcorner[0]
    if dcorner[1] < 0:
        source = source[..., :dcorner[1], :]
    else:
        padding[0][1] = dcorner[1]

    corner2 = utils.pixel_to_pixel(target_wcs, source_wcs, 0, 0)
    # Sanity check---since the images are on the same pixel grids, the corner's coordinates should come out as integers
    np.testing.assert_allclose(np.rint(corner2), corner2)
    corner2 = np.rint(corner2).astype(int)

    if corner2[0] > 0:
        source = source[..., :, corner2[0]:]
    else:
        padding[1][0] = -corner2[0]
    if corner2[1] > 0:
        source = source[..., corner2[1]:, :]
    else:
        padding[0][0] = -corner2[1]

    if corner2[0] > 0 and corner[0] < 0:
        # This can occur when there's no overlap between the two frames.
        padding[1] = [0, target.shape[-1]]

    if np.any(padding != 0):
        if source.ndim > 2:
            padding = np.concatenate(([(0, 0)] * (source.ndim - 2), padding), axis=0)
        source = np.pad(source, padding, constant_values=fill_value)
    return source
