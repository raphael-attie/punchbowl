...
def level3_PIM_flow(data_list: list[str] | list[NDCube],  # noqa: N802
    ...
    out_list.meta.provenance = [fname for d in data_list
...
def level3_core_flow(data_list: list[str] | list[NDCube],
    ...
    data_list.meta.provenance = [fname for d in data_list
        if (fname := d.meta.get("FILENAME"))]
...
def generate_level3_low_noise_flow(data_list: list[str] | list[NDCube],
    ...
    low_noise_image.meta.provenance = [fname for d in data_list
        if (fname := d.meta.get("FILENAME"))]
